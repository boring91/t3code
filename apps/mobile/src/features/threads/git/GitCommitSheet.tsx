...
export function GitCommitSheet(_props: GitCommitSheetProps) {
  ...
  const allFiles = supportsChanges
    ? (changes.data?.staged ?? [])
  ...
  const runCommitAction = useCallback(
    ...
    [allSelected, dialogCommitMessage, gitActions, navigation, selectedFiles, supportsChanges],
  );
            ...
            <View className="flex-row items-center justify-between gap-3">
              <View className="gap-1">
                ...
                <Text className="text-foreground-muted text-xs leading-normal">
                  {selectedFiles.length} {supportsChanges ? "staged" : "selected"} · +
